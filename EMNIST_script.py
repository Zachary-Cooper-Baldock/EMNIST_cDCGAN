...
Test_Name = f"EMNIST_{train_epoch}Epoch_{optimiser}Optim"
Dataset = "EMNIST"
...
#########################################################
# FID Metric & Helper Functions
#########################################################
fid_metric = FrechetInceptionDistance(feature=2048).to(device)

def to_3channels(batch: torch.Tensor) -> torch.Tensor:
    """
    Replicate 1-channel images to 3 channels for Inception-based FID.
    E.g., [N,1,H,W] -> [N,3,H,W].
    """
...
def float_to_uint8(tensor_float: torch.Tensor, assume_neg1_to1=True) -> torch.Tensor:
    """
    Converts float images from [-1,1] or [0,1] range to [0..255] uint8.
    If images are in [-1,1], set assume_neg1_to1=True (default).
    """
    if assume_neg1_to1:
        # [-1,1] -> [0,1]
...
#########################################################
# Custom Disparity-Weighted MSE (GMSE) Loss
#########################################################
...
train_loader = torch.utils.data.DataLoader(
    datasets.EMNIST('./data/EMNIST', split='letters', train=True, download=True, transform=transform),
    batch_size=batch_size, shuffle=True
)

test_loader = torch.utils.data.DataLoader(
    datasets.EMNIST('./data/EMNIST', split='letters', train=False, download=True, transform=transform),
    batch_size=batch_size, shuffle=True
)
...
#########################################################
G = ConditionalGenerator(d=128, num_classes=27, embedding_dim=50)
D = ConditionalDiscriminator(d=64, num_classes=27, embedding_dim=50)
...
for epoch in range(train_epoch):
    ...
    for batch_idx, (x_, labels) in enumerate(train_loader):
        ...
        # Additional losses (not necessarily used in combined objective)
        ...
        # FID (convert to 3-ch, then uint8)
        with torch.no_grad():
    ...
    with torch.no_grad():
        for batch_idx, (x_, labels) in enumerate(test_loader):
            ...
            G_test_GMSE_loss = GMSE_loss(G_fake, x_, 1.0, 2.0)
            
            G_test_loss = G_test_BCE_loss + lambda_term * G_test_MSE_loss # not adding MSE or SSIM here
    ...
    # Print logging
    print('[%d/%d] - ptime: %.2f, loss_d: %.4f, loss_g: %.4f, '
    ...
    # Create a set of fixed labels (0..26) for generating samples each epoch
    fixed_labels = torch.arange(0, 27)[:25].to(device)
...
# Animated GIF of generated samples
images = []
